...
describe("activityService", () => {
  it("records activity and lists newest first, optionally filtered by user", async () => {
    ...
    expect(all.activities.map((row) => row.userId)).toEqual(["usr_al", "usr_ada"]);
    expect(all.activities[1]?.data).toEqual({ postId: "pst_1" });
    ...
    expect(skipped.ok && skipped.recorded).toBe(false);
  });
});
