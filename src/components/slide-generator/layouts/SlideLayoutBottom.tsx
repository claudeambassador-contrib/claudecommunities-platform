...
export default function SlideLayoutBottom({ template, speaker }: LayoutProps) {
  ...
  const previewHead = headSize * 0.55;

  return (
    <>
      {vis.header && (
        <div data-slide-el="header" className="absolute top-[6%] left-[5%] right-[5%]">
          <div
            style={{
      ...
      {eventDate && (
        <div data-slide-el="eventDate" className="absolute top-[12%] left-[5%] right-[5%]">
          <EventDateInline template={template} />
        </div>
      )}

      <div className="absolute bottom-[8%] left-[5%] right-[5%] flex items-end gap-[3%]">
        {vis.headshot && (
        ...
        {vis.logo && speaker?.company_logo_url && (
          <div
            data-slide-el="logo"
            className="shrink-0 w-8 h-8 rounded-md overflow-hidden bg-white/10 flex items-center justify-center"
          >
            <SlideImage
