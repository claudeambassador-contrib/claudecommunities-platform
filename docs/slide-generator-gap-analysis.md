...
| aspect ratio). Per-element _scale_ was removed — the legacy `scale` field is still rendered for back-compat but no longer settable. |
...
first drag _or resize_, the editor measures the live `getBoundingClientRect`
...
sets `width` _and_ a `wrap` flag, and the renderers only re-wrap text when
...
For layouts whose flex headshot has a _non-default_ footprint — split's
