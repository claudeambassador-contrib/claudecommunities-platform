...
| Phase           | Status      | Notes                                                                                                                                                                                                                                                                                                                      |
| --------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0 Scaffold      | Done        | Start + Clerk + CF Vite + oxlint/oxfmt (Ultracite presets)                                                                                                                                                                                                                                                                 |
...
| 4 Worker parity | In progress | Workflows exported from `server.ts`. Cron drains social posts and due email campaigns. `CampaignSendWorkflow` calls `sendCampaign` + Resend. MCP Streamable HTTP at `/mcp`. Staging Worker is live; secrets + authenticated smoke still owed.                                                                              |
...
| 6 Cutover       | In progress | Clean-slate Start D1s (no ETL). Root Next/Prisma/OpenNext tree removed. Staging Worker deployed; secret put + login/campaign/MCP/upload/social smoke still owed.                                                                                                                                                         |
